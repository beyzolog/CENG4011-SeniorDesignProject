...
genes = ["MYC", "CTNNB1"]
BASE_DATA_DIR = "../data"
input_folder = os.path.join(BASE_DATA_DIR, "04_raw_bit_matrices")   # Sızıntısız 2048 bitlik orijinal matris
output_folder = os.path.join(BASE_DATA_DIR, "05_updated_selected_features")
...
def remove_multicollinearity(X_train, selected_features, threshold=0.80):
    """
    %80+ korelasyonlu feature'lardan birini tut, diğerlerini ele.
    (Data Leakage) önlemek için SADECE X_train üzerinden hesaplanır.
    """
    ...
    # %80+ korelasyonlu feature'ları bul
...
for gene in genes:
    ...
    # Veriyi yükle (Artık sızıntısız 2048 bitlik orijinal matrisi okuyor)
    file_path = os.path.join(input_folder, f"{gene}_ecfp4_2048_original.csv")
    ...
    # 1. Train-Test Split (80-20, stratified) - Sızıntı korumasının ilk adımı
    ...
    # 5. Sonuçları kaydet (TÜM DATA ile ama sadece optimal sızıntısız feature'lar)
